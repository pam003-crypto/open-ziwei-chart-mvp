...
注意：GitHub Pages 是静态托管，不能执行 Next.js API Route。页面提供了“AI 设置”，可切换为自定义代理 API 或自定义 API 模式进行预览。自定义 API 模式允许用户自行填写 Base URL、模型和 API Key；Base URL 默认 `https://api.openai.com/v1`，系统会自动调用 `/responses`。公开站点不建议在浏览器中长期保存 API Key；更稳妥的方式是部署到 Vercel / Node 服务，并把 Key 放在服务器环境变量里。
...
- 本地规则解读
- 基于本地规则结果的 AI 解读入口
- 网页端 AI 设置：服务端默认 / 自定义代理 / 自定义 API / Base URL
